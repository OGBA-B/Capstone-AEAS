...
*/
/**************************************************************************/
uint32_t Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout)
{
  uint32_t cid;
...
uint32_t Adafruit_PN532::readPassiveTargetID1(uint8_t cardbaudrate,uint16_t timeout)
{
  uint32_t cid;
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
  pn532_packetbuffer[1] = 1;  // max 1 cards at once (we can set this to 2 later)
  ...
  //Serial.print("Found ");
  //Serial.prinreadPassiveTargetID1t(pn532_packetbuffer[7], DEC);
  //Serial.println(" tags");
  if (pn532_packetbuffer[7] != 1)
    return 0;
